...
class _TechnicianProfileScreenState extends State<TechnicianProfileScreen> {
  ...
  void initState() {
    super.initState();
    _technicianFuture = FirebaseFirestore.instance.collection('technicians').doc(widget.technicianId).get();
  }
    ...
    return Scaffold(
        ...
        body: FutureBuilder<DocumentSnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            final rating = (double.tryParse(techData['rating']?.toString() ?? '0') ?? 0.0).toStringAsFixed(1);
            final reviews = techData['reviews']?.toString() ?? '(0)';
            final price = services.isNotEmpty ? "\$${services[0]['rate']}" : "\$0.00";
            final imagePath = techData['profilePicture'] ?? 'assets/sample_6.png'; // Use a placeholder
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 8),
                          Text("$category - 5 years Experience", style: const TextStyle(color: Colors.grey, fontSize: 14)),
          ...
          children: [
            ...
            TextButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => TechnicianReviewsScreen(techName: techName)),
                );
              },
            ...
            Column(
              ...
              children: [
                ...
                Text.rich(TextSpan(children: [
                  TextSpan(text: price, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                  const TextSpan(text: "/Hr", style: TextStyle(color: Colors.grey, fontSize: 14)),
                ])),
              ],
            ),
            ElevatedButton(
              onPressed: () => ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Booking flow for $name coming soon!"))),
              style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 48, vertical: 16), backgroundColor: Colors.blue.shade700, foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
